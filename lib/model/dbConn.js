...
function init(host, db, port, username, password, options, callback) {
    ...
    if (options.replSet) {
        ...
        url = 'mongodb://root:mongodb@mongodb-rs-kubecity:27017/iotaul?authSource=admin';
    }
    ...
    function connectionAttempt(url, options, callback) {
        ...
        candidateDb = mongoose.createConnection(url, options, function(error, result) {
            if (error) {
                logger.error(context, 'MONGODB-001: Error trying to connect to MongoDB: %s' + '' + url + ' ' + options.replSet.rs_name, error);
                lastError = error;
            } else {
