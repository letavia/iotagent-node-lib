...
function init(host, db, port, username, password, options, callback) {
    ...
    if (options.replicaSet) {
        //url += '?replicaSet=' + options.replicaSet.rs_name;
         url = 'mongodb://root:mongodb@mongodb-rs-kubecity:27017/iotaul?replicaSet=kubecity-rs';
    }
    ...
    function connectionAttempt(url, options, callback) {
        ...
        candidateDb = mongoose.createConnection(url, options, function(error, result) {
            if (error) {
                logger.error(context, 'MONGODB-001: Error trying to connect to MongoDB: %s' + hosts + db + port + username + password + options.replicaSet.rs_name, error);
                lastError = error;
            } else {
    ...
    if ((currentConfig.deviceRegistry && currentConfig.deviceRegistry.type === 'mongodb') ||
        ...
        if (!currentConfig.mongodb || !currentConfig.mongodb.host) {
            ...
            if (currentConfig.mongodb.replicaSet) {
                options.replicaSet = { rs_name: currentConfig.mongodb.replicaSet }; 
                //{ rs_name: 'kubecity-rs' };
            }
