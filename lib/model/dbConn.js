...
function init(host, db, port, username, password, options, callback) {
    ...
    //url = 'mongodb://root:mongodb@mongodb-rs-kubecity:27017/iotaul';

    //if (options.replSet) {
        //url += '?replicaSet=' + options.replSet.rs_name;
        url = 'mongodb://root:mongodb@mongodb-rs-kubecity:27017/iotaul?authSource=admin&replicaSet=kubecity-rs';
        //logger.info(config.getConfig().mongodb.host+dbName+port+currentConfig.username+currentConfig.password+options.replSet.rs_name);
    //}
    ...
    function connectionAttempt(url, options, callback) {
        ...
        candidateDb = mongoose.createConnection(url, options, function(error, result) {
            if (error) {
                logger.error(context, 'MONGODB-001: Error trying to connect to MongoDB: %s' + '' + url + ' ', error);
                lastError = error;
            } else {
    ...
    if ((currentConfig.deviceRegistry && currentConfig.deviceRegistry.type === 'mongodb') ||
        ...
        if (!currentConfig.mongodb || !currentConfig.mongodb.host) {
            ...
            if (currentConfig.mongodb.replicaSet) {
                options.replSet = { rs_name: 'kubecity-rs' };
                    //{ rs_name: currentConfig.mongodb.replicaSet }; 
                //{ rs_name: 'kubecity-rs' };
            }

            init('mongodb-rs-kubecity', 'iotaul', 27017, 'root', 'mongodb', options, callback);
            logger.info(currentConfig.mongodb.host+dbName+port+currentConfig.mongodb.username+currentConfig.mongodb.password+options.replSet.rs_name);
        }
    } else {
        callback();
    }
